...
public class TakingTurnsQueueTests
{
    ...
    // Defect(s) Found: PersonQueue.Enqueue() inserts at front (index 0) instead of back, causing wrong order.
    // TakingTurnsQueue.GetNextPerson() only re-enqueues if turns > 1, missing cases where turns == 1 or turns <= 0.
    ...
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: PersonQueue.Enqueue() inserts at front (index 0) instead of back, causing wrong order.
    // TakingTurnsQueue.GetNextPerson() only re-enqueues if turns > 1, missing cases where turns == 1 or turns <= 0 (infinite).
