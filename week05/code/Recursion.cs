...
public static class Recursion
{
    ...
    {
        // Base case: if we've reached the desired size, add the word to results
        if (word.Length == size)
        {
            results.Add(word);
            return;
        ...
        // For each letter in the remaining letters, add it to the word and recurse
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        // If no wildcard found, the pattern itself is the result
        if (wildcardIndex == -1)
        {
            results.Add(pattern);
            return;
        ...
        // Recursively solve for both 0 and 1 replacements
